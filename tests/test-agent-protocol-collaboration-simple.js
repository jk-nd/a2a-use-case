...
/**
 * Simplified Real-world test: Agent Collaboration with Protocol Instantiation
 * 
 ...
 * 
 * This version assumes the payment protocol is already deployed
 */
async function testAgentProtocolCollaborationSimple() {
    console.log('🚀 Testing Agent-Protocol Collaboration Workflow (Simplified)...\n');
    ...
    try {
        ...
        // Step 2: Check if Payment Protocol is Available
        console.log('📋 Step 2: Check Payment Protocol Availability');
        const protocolAvailable = await checkProtocolAvailability(token);
        if (!protocolAvailable) {
            console.log('⚠️  Payment protocol not available, skipping protocol operations');
            console.log('✅ Continuing with agent communication only\n');
        } else {
            console.log('✅ Payment protocol is available\n');
        }
        ...
        console.log('📋 Purchase request structure:', JSON.stringify(purchaseRequest, null, 2));
        ...
        // Step 5: Agents Collaborate (with or without protocol)
        if (protocolAvailable) {
            ...
            console.log('📋 Step 5: Agents Collaborate (Communication Only)');
            await simulateCollaboration(buyerAgent, sellerAgent, purchaseRequest, quoteResponse);
        ...
        await communicateFinalResults(buyerAgent, sellerAgent, protocolAvailable ? { protocolId: 'simulated' } : null);
        ...
        if (protocolAvailable) {
            ...
            console.log('✅ Agent collaboration via communication only');
        }
 ...
 */
async function checkProtocolAvailability(token) {
    try {
        // Try to list protocols to see if payment_workflow is available
...
async function sellerRespondsWithQuote(sellerAgent, buyerAgent, purchaseRequest) {
    ...
    const quoteResponse = await axios.post(`${A2A_SERVER_URL}/agents/message`, {
        ...
        content: {
            ...
            quoteDetails: {
                quoteId: `QT-${Date.now()}`,
                orderId: purchaseRequest.response.echo.orderDetails.orderId,
                items: purchaseRequest.response.echo.orderDetails.items,
                totalAmount: 7000,
                discount: 500,
                finalAmount: 6500,
...
/**
 * Simulate collaboration without protocol
 */
async function simulateCollaboration(buyerAgent, sellerAgent, purchaseRequest, quoteResponse) {
    // Step 1: Buyer accepts quote
    await axios.post(`${A2A_SERVER_URL}/agents/message`, {
        ...
        content: {
            type: 'quote_accepted',
            originalRequestId: purchaseRequest.messageId,
            quoteId: quoteResponse.response.echo.quoteDetails.quoteId,
            orderId: purchaseRequest.response.echo.orderDetails.orderId,
            acceptedAmount: quoteResponse.response.echo.quoteDetails.finalAmount
        }
    });
    ...
    await axios.post(`${A2A_SERVER_URL}/agents/message`, {
        ...
        content: {
            ...
            estimatedDelivery: quoteResponse.response.echo.quoteDetails.estimatedDelivery
        }
    });
...
async function createProtocolInstance(buyerAgent, sellerAgent, purchaseRequest, quoteResponse, token) {
    ...
    const createOrderResponse = await axios.post(`${A2A_SERVER_URL}/a2a/method`, {
        ...
        params: {
            orderDetails: {
                orderId: purchaseRequest.response.echo.orderDetails.orderId,
                totalAmount: quoteResponse.response.echo.quoteDetails.finalAmount,
                deliveryDate: quoteResponse.response.echo.quoteDetails.estimatedDelivery,
                buyer: purchaseRequest.response.echo.buyerInfo.company,
                seller: quoteResponse.response.echo.sellerInfo.company
            }
        },
        token: token
    });
...
async function communicateFinalResults(buyerAgent, sellerAgent, workflowResult) {
    ...
    await axios.post(`${A2A_SERVER_URL}/agents/message`, {
        ...
        content: {
            ...
            protocolId: workflowResult?.protocolId || 'simulated',
            orderId: 'PO-' + Date.now(),
            finalState: workflowResult?.finalState || 'completed',
...
if (require.main === module) {
    testAgentProtocolCollaborationSimple()
        .then(result => {
            if (result.success) {
...
module.exports = { testAgentProtocolCollaborationSimple }; 